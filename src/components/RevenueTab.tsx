...
export default function RevenueTab() {
  const { revenue, setRevenue, journal } = useStore();
  const [year, setYear] = useState(new Date().getFullYear());
  const [month, setMonth] = useState(new Date().getMonth() + 1);

  const get = (y: number, m: number, key: string) => revenue[`${y}-${m}-${key}`] || 0;

  // Keys for the two auto-fed types
  const KEY_REGISTRATION = typeKey(3, 2, 3, 7); // رسوم استمارات وتسجيل
  const KEY_EXAMS = typeKey(3, 2, 3, 8);        // رسوم امتحانات وشهادات

  // Derived amounts from journal entries: aggregate credit by month based on
  // description keywords ("دراسي" → registration, "اختبار" → exams).
  const derived = useMemo(() => {
    ...
    journal.forEach((j) => {
      ...
      if (desc.includes("دراسي")) key = KEY_REGISTRATION;
      else if (desc.includes("اختبار")) key = KEY_EXAMS;
      if (!key) return;
      const k = `${y}-${m}-${key}`;
      out[k] = (out[k] || 0) + credit;
    });
    return out;
  }, [journal, KEY_REGISTRATION, KEY_EXAMS]);

  const getEffective = (y: number, m: number, key: string) => {
    const stored = revenue[`${y}-${m}-${key}`];
    if (stored !== undefined && stored !== 0) return stored;
    return derived[`${y}-${m}-${key}`] || 0;
  };
  ...
  const data = useMemo(() => {
    const sumPrev = (key: string) => {
      let s = 0;
      for (let m = 1; m < month; m++) s += getEffective(year, m, key);
      return s;
    };
      ...
      ch.sections.forEach((sec) =>
        sec.items.forEach((it) =>
          it.types.forEach((t) => {
            ...
            const cur = getEffective(year, month, k);
                        ...
                        {sec.items.map((it) => (
                          <Fragment key={`it-${ch.no}-${sec.no}-${it.no}`}>
                            ...
                            {it.types.map((t) => {
                              ...
                              const stored = revenue[`${year}-${month}-${k}`];
                              const fromJournal = derived[`${year}-${month}-${k}`] || 0;
                              const isAuto = (stored === undefined || stored === 0) && fromJournal > 0;
                              return (
                                <tr key={k} className="hover:bg-muted/30">
                                  ...
                                  <td className="border px-1 py-0.5">
                                    <input
                                      ...
                                      className={`w-full px-2 py-1 border rounded font-mono text-left focus:outline-none focus:ring-1 focus:ring-ring ${isAuto ? "bg-success/10 text-success-foreground" : "bg-input/30"}`}
                                      placeholder={isAuto ? fmt(fromJournal) : "0"}
                                      title={isAuto ? `محسوب تلقائياً من اليومية: ${fmt(fromJournal)}` : undefined}
                                    />
                                  </td>
