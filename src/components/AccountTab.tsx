...
const Modal = ({ title, isOpen, onClose, children }: { title: string; isOpen: boolean; onClose: () => void; children: React.ReactNode }) => {
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-end sm:items-center justify-center z-50 p-2 sm:p-4 animate-fadeIn">
...
export default function AccountsTab() {
  ...
  const handleSyncFromHafiza = async () => {
    const source = (hafiza && hafiza.length > 0) ? hafiza : [];
    ...
    currentAccounts.forEach((acc: any) => {
      if (acc.sourceHafizaId) {
        byHafizaId.set(String(acc.sourceHafizaId), acc);
        linkedAccountIds.add(acc.id);
      }
    });

    currentAccounts.forEach((acc: any) => {
      if (acc.hafizaNo && !linkedAccountIds.has(acc.id) && !byHafizaNo.has(String(acc.hafizaNo))) {
        byHafizaNo.set(String(acc.hafizaNo), acc);
      }
    });

    source.forEach((hafizaRow: any) => {
      if (!hafizaRow?.id) return;
      const hid = String(hafizaRow.id);

      const notifyAmountValue = hafizaRow.notifyAmount ?? hafizaRow.supplyAmount ?? hafizaRow.tawreedAmount ?? 0;
      const incomeValue = parseAmount(notifyAmountValue);

      const mappedData = {
        date: hafizaRow.date || today(),
        hafizaNo: String(hafizaRow.hafizaNo),
        notifyNo: String(hafizaRow.notifyNo),
        notifyDate: hafizaRow.notifyDate || "",
        description: String(hafizaRow.description),
        specialty: String(hafizaRow.specialty),
        name: String(hafizaRow.name),
        hafizaAmount: parseAmount(hafizaRow.hafizaAmount || hafizaRow.amount),
        income: incomeValue,
      };
      ...
      const hasDiff =
        existing.date !== mappedData.date ||
        existing.hafizaNo !== mappedData.hafizaNo ||
        existing.notifyNo !== mappedData.notifyNo ||
        existing.notifyDate !== mappedData.notifyDate ||
        existing.description !== mappedData.description ||
        existing.specialty !== mappedData.specialty ||
        existing.name !== mappedData.name ||
        existing.hafizaAmount !== mappedData.hafizaAmount ||
        existing.income !== mappedData.income ||
    ...
    if (addedCount > 0 || updatedCount > 0) {
      ...
      toast.info(`جميع السجلات متطابقة.`);
    }
  };
  ...
  const handleImportExcel = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        if (json.length === 0,notifyNo: e.target.value})} className="w-full p-2 text-sm border border-slate-300 rounded-xl outline-none" /></div>
              ...
              <div><label className="block text-xs font-bold text-slate-500 mb-1">رقم الشيك</label><input value={editingRow.checkNo} onChange={(e) => setEditingRow({...editingRow, checkNo: e.target.value})} className="w-full p-2 text-sm border border-slate-300 rounded-xl outline-none" /></div>
              <div><label className="block text-xs font-bold text-slate-500 mb-1">تاريخ الشيك</label><input type="date" value={editingRow.checkDate} onChange={(e) => setEditingRow({...editingRow, checkDate: e.target.value})} className="w-full p-2 text-sm border border-slate-300 rounded-xl outline-none" /></div>
              <div><label className="block text-xs font-bold text-slate-500 mb-1">البيان</label><input value={editingRow.description} onChange={(e) => setEditingRow({...editingRow, description: e.target.value})} className="w-full p-2 text-sm border border-slate-300 rounded-xl outline-none" /></div>
          ...
          <div className="p-4 bg-slate-50/40">
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-6 gap-3 items-end">
              ...
              <div className="sm:col-span-2">
                <label className="block text-xs font-bold text-[#10528e] mb-1.5 mr-1 flex items-center gap-1"><Link className="w-3.5 h-3.5" /> ربط بدليل هيكل الإيرادات</label>
              ...
              <table className="w-full text-xs sm:text-sm text-right border-collapse border border-black table-auto">
                ...
                <tbody className="text-slate-700 font-medium bg-white">
                  {filteredWithBalance.length === 0 ? (
                    <tr>
                      <td colSpan={COLS.length + 2} className="p-12 text-center text-slate-500 font-bold border border-black">لا توجد بيانات تطابق مرشحات البحث.</td>
                    </tr>
                  ) : (
