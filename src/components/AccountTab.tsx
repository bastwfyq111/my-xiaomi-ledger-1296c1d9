...
export default function AccountsTab() {
  ...
  // حساب الرصيد بناءً على كل البيانات
      ...
      try {
        ...
        const importedAccounts = json
          .map((row: any) => {
            ...
            return {
              date: cleanRow["التاريخ"] || cleanRow["date"] || today(),
              hafizaNo: String(cleanRow["رقم الحافظة"] || ""),
              notifyNo: String(cleanRow["رقم الإشعار"] || ""),
              notifyDate: cleanRow["تاريخ التوريد"] || "",
              checkNo: String(cleanRow["رقم الشيك"] || ""),
              checkDate: cleanRow["تاريخ الشيك"] || "",
              description: cleanRow["البيان"] || "",
              specialty: cleanRow["التخصص"] || "",
              name: cleanRow["الاسم"] || "",
              hafizaAmount: parseAmount(cleanRow["مبلغ الحافظة"]),
              income: parseAmount(cleanRow["الإيرادات"]),
              expense: parseAmount(cleanRow["المصروفات"]),
              revenueKey: String(cleanRow["رمز الإيراد"] || ""),
            };
          });

        useStore.getState().importData({ accounts: importedAccounts });
        toast.success(`تم استيراد ${importedAccounts.length} سجل`);
      } catch (err) {
        toast.error("خطأ في استيراد ملف الإكسل");
      }
    };
    ...
    <div className="w-full space-y-6" dir="rtl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-4">
        ...
        <div className="bg-gradient-to-br from-[#10528e] to-[#0b3d6d] p-5 rounded-2xl text-white shadow-md flex items-center justify-between">
          ...
          <div className="p-3.5 bg-white/10 rounded-2xl"><Wallet className="w-6 h-6" /></div>
        </div>
      </div>

      {/* قالب الإدخال */}
      <div className="w-full bg-white shadow-sm border border-slate-100 rounded-2xl overflow-hidden mb-4">
        {/* رأس القسم مع أزرار العمليات */}
        <div className="bg-gradient-to-r from-[#10528e] to-[#0f467a] px-5 py-4 flex flex-col sm:flex-row sm:justify-between items-center gap-4 border-b">
          <div className="flex items-center gap-2.5 text-white mb-2 sm:mb-0">
            <Plus className="w-4 h-4" />
            <h2 className="text-sm font-bold">إضافة حركة مالية</h2>
          </div>
          {/* الأزرار بشكل منسق ومتوازن */}
          <div className="flex gap-3 flex-wrap justify-center sm:justify-end">
            <button onClick={handleSyncFromHafiza} className="flex items-center gap-1.5 px-4 py-2 bg-amber-500 hover:bg-amber-600 text-white rounded-xl text-xs font-bold shadow-sm transition-all">
              <RefreshCw className="w-3.5 h-3.5" /> <span>مطابقة 2026 ⚡</span>
            </button>
            <label className="flex items-center gap-1.5 px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-bold cursor-pointer shadow-sm transition-all">
        ...
        <div className="p-4 bg-slate-50/40">
          <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-6 gap-3 items-end">
            ...
            {/* أزرار الإدخال */}
            <div className="sm:col-span-2 flex gap-3 pt-2 justify-center sm:justify-start">
              <button onClick={() => {
                addAccount({ ...form, hafizaAmount: Number(form.hafizaAmount) || 0, income: Number(form.income) || 0, expense: Number(form.expense) || 0 });
                setForm(emptyForm);
                toast.success("تم الحفظ");
              }} className="flex items-center gap-2 px-4 py-2 bg-[#10528e] hover:bg-[#0b3d6d] text-white rounded-xl text-xs font-bold shadow-sm transition-all">
                <Save className="w-4 h-4" /> حفظ
              </button>
              <button onClick={() => setForm(emptyForm)} className="flex items-center gap-2 px-4 py-2 bg-slate-200 hover:bg-slate-300 text-slate-700 rounded-xl text-xs font-bold shadow-sm transition-all">
      ...
      <div className="w-full bg-white shadow-sm border border-black rounded-xl overflow-hidden">
        {/* رأس الجدول مع أدوات التحكم */}
        <div className="bg-slate-800 px-5 py-3 flex flex-col sm:flex-row sm:justify-between items-center gap-4">
          <h2 className="text-sm font-bold text-white">جدول الحساب ({accounts.length})</h2>
          <div className="flex gap-3 flex-wrap justify-center sm:justify-end">
            {Object.values(filters).some(Boolean) && (
              <button onClick={clearFilters} className="px-3 py-1 bg-rose-600 text-white rounded-lg text-xs shadow-sm transition-all hover:bg-rose-700">مسح الفلاتر</button>
            )}
        ...
        <div className="overflow-x-auto max-h-[600px]">
          <table className="w-full text-xs text-right border-collapse border border-black">
            <thead className="sticky top-0 z-20 bg-slate-100">
              <tr>
                <th className="p-2 border border-black w-10">م</th>
                {COLS.map(c => (
                ...
                <th className="p-2 border border-black w-20">إجراءات</th>
              </tr>
              {/* مرشحات */}
              <tr className="bg-slate-50">
                ...
                {COLS.map(c => (
                  <th key={c.key} className="p-1 border border-black">
                    <input value={filters[c.key] || ""} onChange={(e) => setFilter(c.key, e.target.value)} placeholder="تصفية..." className="w-full p-1 border rounded text-[10px]" />
                  </th>
                ))}
                <th className="border border-black"></th>
              </tr>
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((acc, index) => (
                  <tr key={acc.id} className="hover:bg-slate-50">
                    ...
                    <td className="p-2 border border-black">{acc.date}</td>
                    ...
                    <td className="p-2 border border-black">{fmt(acc.hafizaAmount)}</td>
                    <td className="p-2 border border-black bg-emerald-50/30 text-center font-bold text-emerald-700">{fmt(acc.income)}</td>
                    <td className="p-2 border border-black bg-rose-50/30 text-center font-bold text-rose-700">{fmt(acc.expense)}</td>
                    {/* ربط رمز الإيراد */}
                    <td className="p-2 border border-black">
                      <select
                        value={acc.revenueKey || ""}
                        onChange={(e) => {
                          updateAccount(acc.id, { ...acc, revenueKey: e.target.value || undefined });
                    ...
                    {/* الرصيد */}
                    <td className="p-2 border border-black text-center bg-blue-50/30 font-black text-[#10528e]">{fmt(acc.balance)}</td>
                    {/* إجراءات */}
                    <td className="p-2 border border-black">
        ...
        {editingRow && (
          <form onSubmit={(e) => {
            ...
            <div className="flex justify-end gap-2 pt-4">
              ...
              <button type="submit" className="px-4 py-2 bg-[#10528e] text-white rounded-xl">حفظ</button>
            </div>
          </form>
        )}
      </Modal>
