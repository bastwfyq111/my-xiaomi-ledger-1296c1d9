...
function Field({ label, v, on, type = "text", placeholder = "", icon, className = "" }: FieldProps) {
  return (
...
export default function AccountsTab() {
  ...
  const handleImportExcel = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!validExtensions.includes(fileExtension)) {
      toast.error("يرجى اختيار ملف Excel صالح (xlsx, xls, csv)");
      e.target.value = "";
      return;
    }
      ...
      try {
        ...
        const importedAccounts = json
          .map((row: any) => {
            ...
            return {
              date: formatDate(cleanRow["التاريخ"] || cleanRow["تاريخ"] || cleanRow["date"] || cleanRow["Date"]) || today(),
              hafizaNo: String(cleanRow["رقمالحافظة"] || cleanRow["رقم الحافظة"] || cleanRow["hafizaNo"] || ""),
              notifyNo: String(cleanRow["رقمالإشعار"] || cleanRow["رقم الإشعار"] || cleanRow["notifyNo"] || ""),
              notifyDate: formatDate(cleanRow["تاريخالتوريد"] || cleanRow["تاريخ التوريد"] || cleanRow["notifyDate"] || ""),
              checkNo: String(cleanRow["رقمالشيك"] || cleanRow["رقم الشيك"] || cleanRow["checkNo"] || ""),
              checkDate: formatDate(cleanRow["تاريخالشيك"] || cleanRow["تاريخ الشيك"] || cleanRow["checkDate"] || ""),
              description: String(cleanRow["البيان"] || cleanRow["description"] || ""),
              specialty: String(cleanRow["التخصص"] || cleanRow["specialty"] || ""),
              name: String(cleanRow["الاسم"] || cleanRow["name"] || ""),
              hafizaAmount: formatNumber(cleanRow["مبلغالحافظة"] || cleanRow["مبلغ الحافظة"] || cleanRow["hafizaAmount"]),
              income: formatNumber(cleanRow["الإيرادات"] || cleanRow["الايرادات"] || cleanRow["income"]),
              expense: formatNumber(cleanRow["المصروفات"] || cleanRow["المصروف"] || cleanRow["مصروفات"] || cleanRow["مصروف"] || cleanRow["expense"]),
              revenueKey: String(cleanRow["رمزالإيراد"] || cleanRow["رمز الإيراد"] || cleanRow["revenueKey"] || ""),
            };
          })
          .filter((row: any) => row.name || row.description || row.income > 0 || row.expense > 0);

        if (importedAccounts.length === 0) {
          toast.warning("لم يتم العثور على بيانات صالحة للاستيراد");
          return;
        }

        // استخدام دالة importData من الـ store
        ...
        toast.error(`فشل استيراد ملف الإكسل: ${err.message || "خطأ غير معروف"}`);
      }
    };
      ...
      <div className="w-full bg-white shadow-sm border border-slate-100 rounded-2xl overflow-hidden">
        <div className="bg-gradient-to-r from-[#10528e] to-[#0f467a] px-5 py-4 flex flex-wrap justify-between items-center gap-4 border-b">
          <div className="flex items-center gap-2.5">
            ...
            <h2 className="text-sm sm:text-base font-bold text-white">إضافة حركة مالية يدويّة أو استيراد من Excel</h2>
          </div>
            ...
            <table className="w-full text-xs sm:text-sm text-right border-collapse border border-black table-auto">
              ...
              <tbody className="text-slate-700 font-medium bg-white">
                {filteredWithBalance.length === 0 ? (
                  <tr>
                    <td colSpan={COLS.length +, 2} className="p-12 text-center text-slate-500 font-bold border border-black">
