...
// تعريف الحقول الأساسية لعام 2025
const BASE_COLS = [
...
export default function InstallmentsTab() {
  ...
  // دالة تطهير الأرقام من الفواصل والنصوص الغريبة لضمان سلامة العمليات الحسابية
  ...
  // تهيئة مستند PDF يدعم محاذاة اللغة العربية والخط المخصص
  const createArabicPDF = async (orientation: "portrait" | "landscape" = "landscape") => {
    ...
    if (fontBase64) {
      ...
      console.warn("لم يتم تحميل الخط العربي الافتراضي، قد تظهر الحروف بشكل غير صحيح");
  ...
  // احتساب الإجماليات الشاملة عبر useMemo لحماية الأداء
  ...
  // دالتين نموذجيتين لمعالجة استيراد الملفات الخارجية ومنع الكراش
  const handleImport2025 = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    toast.info("جاري استيراد ومعالجة بيانات عام 2025...");
  };

  const handleImport2026 = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    toast.info("جاري استيراد ومعالجة بيانات عام 2026...");
  };

  // ========== دالة استخراج كشف حساب تفصيلي عمودي A4 يدعم العربية بالكامل ==========
  const printVerticalDetailedStatement = async (studentName: string) => {
    ...
    try {
      // وضعية portrait عمودية لتناسب ورق الطباعة المعتاد A4
      const pdf = await createArabicPDF("portrait");
      ...
      // --- ترويسة المستند ---
      pdf.setFontSize(16);
      pdf.text("المجلس اليمني للاختصاصات الطبية", centerX, currentY, { align: "center" });
      
      currentY += 8;
      pdf.setFontSize(13);
      pdf.text("كشف حساب مالي تفصيلي للمتدرب", centerX, currentY, { align: "center" });
      
      currentY += 5;
      pdf.setLineWidth(0.4);
      pdf.setDrawColor(44, 62, 80);
      pdf.line(marginX, currentY, 210 - marginX, currentY); 
      
      // --- بيانات المتدرب الأساسية ---
      currentY += 10;
      pdf.setFontSize(11);
      pdf.text(`اسم الطبيب المتدرب: ${studentName}`, 210 - marginX, currentY, { align: "right" });
      pdf.text(`تاريخ الاستخراج: ${today()}`, marginX, currentY, { align: "left" });
      
      currentY += 7;
      const batchInfo = r2026?.batch || r2025?.batch || "—";
      const specialtyInfo = r2026?.specialty || r2025?.specialty || "—";
      pdf.text(`الدفعة: ${batchInfo}`, 210 - marginX, currentY, { align: "right" });
      pdf.text(`المساق/التخصص: ${specialtyInfo}`, marginX + 70, currentY, { align: "left" });

      // ==================== تفاصيل عام 2025 م ====================
      if (r2025) {
        currentY += 12;
        pdf.setFontSize(11);
        pdf.text("1. تفاصيل الحساب المالي لعام 2025م:", 210 - marginX, currentY, { align: "right" });
        
        currentY += 4;
        const head2025 = ["البيان المالي لعام 2025", "المبلغ (ريال)"];
        const body2025 = [
          ["إجمالي رسوم العام الدراسي 2025 (عليك)", fmt(superCleanNumber(r2025.fees))],
          ["إجمالي المبالغ المسددة والأقساط (لك)", fmt(superCleanNumber(r2025.totalPaid))],
          ["صافي المتبقي لعام 2025 (عليك)", fmt(superCleanNumber(r2025.remaining))],
        ];

        if (r2025.payments) {
          MONTHS_2025.forEach(month => {
            const paidInMonth = superCleanNumber(r2025.payments[month]);
            if (paidInMonth > 0) {
              body2025.splice(2, 0, [`   - قسط مدفوع لشهر [ ${month} ] (لك)`, fmt(paidInMonth)]);
            }
          });
        }
        ...
        autoTable(pdf, {
          ...
          body: body2025,
          startY: currentY,
          styles: { font: "Cairo", halign: "right", fontSize: 9, cellPadding: 3, textColor: [40, 40, 40] },
          headStyles: { fillColor: [13, 148, 136], textColor: 255, halign: "right" },
          columnStyles: { 0: { cellWidth: 130 }, 1: { cellWidth: 50, halign: "left" } },
      ...
      if (r2026) {
        ...
        pdf.setFontSize(11);
        pdf.text("2. تفاصيل الحساب المالي لعام 2026م (العام الحالي):", 210 - marginX, currentY, { align: "right" });
        
        currentY += 4;
        const head2026 = ["البيان المالي لعام 2026", "المبلغ (ريال)"];
        const body2026 = [
          ["رسوم العام الدراسي الجديد 2026 (عليك)", fmt(superCleanNumber(r2026.fees))],
          ["مستحقات متأخرة ومرحّلة من عام 2025 (عليك)", fmt(superCleanNumber(r2026.prevDue || r2025?.remaining || 0))],
          ["إجمالي المدفوعات والمسدد لعام 2026 (لك)", fmt(superCleanNumber(r2026.totalPaid))],
        ];

        if (r2026.payments) {
          MONTHS_2026_CLEAN.forEach(month => {
            const paidInMonth = superCleanNumber(r2026.payments[month]);
            if (paidInMonth > 0) {
              body2026.push([`   - قسط مدفوع لشهر [ ${month} ] (لك)`, fmt(paidInMonth)]);
            }
          });
        }
      ...
      pdf.setFontSize(12);
      pdf.text("■ الخلاصة المالية النهائية الموحدة:", 210 - marginX, currentY, { align: "right" });
      
      currentY += 4;
      
      const totalAllFees = superCleanNumber(r2025?.fees) + superCleanNumber(r2026?.fees);
      const totalAllPaid = superCleanNumber(r2025?.totalPaid) + superCleanNumber(r2026?.totalPaid);
      const finalRemaining = superCleanNumber(r2026?.remaining ?? r2025?.remaining ?? 0);

      const headSummary = ["صافي المتبقي الحالي النهائي (عليك)", "إجمالي ما تم سداده (لك)", "إجمالي ما على المتدرب (كل الأعوام)"];
      const bodySummary = [[
        fmt(finalRemaining),
        fmt(totalAllPaid),
        fmt(totalAllFees)
      ...
      autoTable(pdf, {
        ...
        body: bodySummary,
        startY: currentY,
        styles: { font: "Cairo", halign: "center", fontSize: 9, cellPadding: 4, fontStyle: "bold" },
      ...
      pdf.setFontSize(10);
      pdf.text("توقيع المحاسب المختص: ....................", 210 - marginX - 5, currentY, { align: "right" });
      pdf.text("ختم الإدارة المالية: ....................", marginX + 45, currentY, { align: "left" });

      const fileName = `كشف_حساب_تفصيلي_${studentName.replace(/\s+/g, '_')}.pdf`;
      pdf.save(fileName);
      toast.success("✅ تم استخراج كشف الحساب التفصيلي الموحد بنجاح");

    } catch (error) {
      console.error("خطأ في تصدير المستند التفصيلي الموحد:", error);
      toast.error("❌ حدث خطأ أثناء إنشاء وتجهيز ملف الـ PDF");
    }
  };

  // تصدير كشف سنوي كامل للعام المحدد
  const printFullYearStatement = async (year: 2025 | 2026) => {
    const data = year === 2025 ? (installments2025 || []) : (installments || []);
    
    if (!data || data.length === 0) {
      toast.error(`لا توجد بيانات متوفرة لتصدير كشف عام ${year}`);
      return;
    }
    ...
    try {
      ...
      autoTable(pdf, {
        ...
        body: body,
        startY: 40,
        styles: { font: "Cairo", halign: "right", fontSize: 8, cellPadding: 2 },
        headStyles: { 
      ...
      console.error("خطأ في التصدير السنوي العام:", error);
      toast.error("❌ فشل تصدير الكشف العام الموحد");
    }
  };

  // معالجة تسجيل قسط يدوي لشهر مالي محدد
  const handleAddManualPayment = (e: React.FormEvent) => {
    e.preventDefault();
    if (!paymentModal || !payAmount || !payMonth) {
      toast.error("يرجى إدخال مبلغ القسط واختيار الشهر مسبقاً");
      return;
    }
    ...
    toast.success(`تم تسجيل قسط بقيمة ${fmt(amountNum)} للشهر المحدد بنجاح`);
      ...
      <div className="bg-card rounded-xl shadow-sm border p-5 bg-white text-right">
        <div className="flex flex-wrap items-center justify-between gap-4 border-b pb-3 mb-4">
          <div>
            ...
            <p className="text-xs text-slate-500">الأرشيف المستورد والمعدّل لعام 2025م</p>
          </div>
        ...
        {/* رندرة جدول 2025 المعتمد على عناصر controls2025 وزر الاستدعاء التفصيلي: */}
        {/* onClick={() => printVerticalDetailedStatement(row.name)} */}
      </div>

      {/* ================== أقسام وعناصر واجهة عام 2026م ================== */}
      <div className="bg-card rounded-xl shadow-sm border p-5 bg-white text-right">
        <div className="flex flex-wrap items-center justify-between gap-4 border-b pb-3 mb-4">
          <div>
            <h2 className="text-base font-bold text-teal-800">أقساط ورسوم عام 2026م (العام الحالي)</h2>
            <p className="text-xs text-slate-500">يتضمن الربط المباشر مع متبقيات السجلات وعمليات السداد الحالية</p>
          </div>
          <div className="flex gap-2">
            <button 
        ...
        {/* رندرة جدول 2026 المعتمد على عناصر controls2026 */}
      </div>
