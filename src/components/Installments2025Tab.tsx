...
const Installments2025Tab: React.FC = () => {
  ...
  // دالة تحديث السجل المفترضة داخل الـ Store لعام 2025 (تأكد من وجودها أو ما يشابهها في الـ store)
  const updateInstallment2025 = useStore((s) => (s as any).updateInstallment2025 || ((name: string, data: any) => {
    // في حال عدم وجود الدالة بالـ store، ستقوم برفع تنبيه للمطور لتسجيلها
    console.warn("يرجى التأكد من إضافة دالة updateInstallment2025 داخل ملف src/lib/store.ts");
  }));

  // حالة التحكم في فتح وإغلاق نافذة التعديل المنبثقة
  ...
  const handleImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    e.target.value = ""; // لإتاحة رفع نفس الملف مجدداً إن لزم الأمر
  };
    ...
    <div className="space-y-4" dir="rtl">
      ...
      <div className="bg-white rounded-xl shadow-sm border p-4 flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">استيراد السجلات، تعديل البيانات وطباعة كشوفات المتدربين</p>
        </div>
        
        <div className="flex flex-wrap items-center gap-3">
          {/* زر رفع واستيراد ملفات الإكسل */}
      ...
      {/* عرض البيانات والجدول الرئيسي */}
      <div className="bg-card rounded-xl shadow-sm border overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-right border-collapse">
            <thead className="bg-slate-100 text-slate-800 font-bold border-b border-slate-200">
              <tr>
                <th className="p-3 text-center border-l w-12 text-slate-500">م</th>
                {INSTALLMENT_2025_COLUMNS.map(col => (
                  <th key={col} className="p-3 border-l text-center whitespace-nowrap">{col}</th>
                ))}
                <th className="p-3 text-center text-teal-900 w-44">الإجراءات والتقارير</th>
              </tr>
            </thead>
            <tbody>
              {installments2025.length === 0 ? (
                <tr>
                  <td colSpan={INSTALLMENT_2025_COLUMNS.length + 2} className="text-center py-10 text-slate-400 font-medium">
                ...
                installments2025.map((row, idx) => (
                  <tr key={row.name + idx} className="border-b bg-white hover:bg-slate-50 transition-colors">
                    <td className="p-2.5 text-center border-l text-slate-400 font-mono">{idx + 1}</td>
                    {INSTALLMENT_2025_COLUMNS.map(col => (
                      <td key={col} className="p-2.5 text-center border-l text-slate-700 font-medium font-sans">
                        {row[col] ?? ''}
                      </td>
                    ))}
                    <td className="p-2 text-center whitespace-nowrap space-x-1 space-x-reverse">
                      {/* زر فتح مودال التعديل */}
                      <button 
                        onClick={() => setEditingIndex(idx)}
                        className="px-2.5 py-1 text-blue-600 hover:text-blue-800 font-bold text-xs bg-blue-50 hover:bg-blue-100 border border-blue-200 rounded-md transition-all ml-1"
                      >
                        تعديل
                      </button>
                      
                      {/* زر طباعة كشف الحساب الخاص بالطالب */}
                      <button 
                        onClick={() => printAccountStatement(row.name)}
                        className="px-2.5 py-1 bg-teal-50 text-teal-700 hover:bg-teal-700 hover:text-white border border-teal-200 rounded-md text-xs font-bold shadow-sm transition-all"
                      >
                        كشف حساب
                      </button>
                    </td>
      ...
      {/* مودال التعديل الفوري عند الضغط على زر تعديل */}
      {editingIndex !== null && (() => {
        const selectedRow = installments2025[editingIndex];
        if (!selectedRow) return null;

        // توليد الحقول ديناميكياً استناداً إلى أعمدة الـ Excel المحددة في الـ Store
        const fields: EditField[] = INSTALLMENT_2025_COLUMNS.map(col => {
          const isNumeric = col.includes("رسوم") || col.includes("مبلغ") || col.includes("المسدد") || col.includes("المتبقي") || col.includes("قسط");
          return {
            key: col,
            label: col,
            type: isNumeric ? "number" : "text",
            colSpan: col === "الاسم" || col === "ملاحظات" ? 2 : 1
          };
        });

        return (
          <EditModal 
            title={`تعديل بيانات المتدرب: ${selectedRow.name || ''}`}
            fields={fields}
            values={selectedRow}
            onClose={() => setEditingIndex(null)}
            onSave={(updatedValues) => {
              // تحويل قيم الحقول الرقمية للتأكد من حفظها كـ الأرقام بدلاً من النصوص
              const finalValues = { ...updatedValues };
              INSTALLMENT_2025_COLUMNS.forEach(col => {
                const isNumeric = col.includes("رسوم") || col.includes("مبلغ") || col.includes("المسدد") || col.includes("المتبقي") || col.includes("قسط");
                if (isNumeric && finalValues[col] !== undefined) {
                  finalValues[col] = Number(finalValues[col]) || 0;
                }
              });

              // إجراء التعديل في الـ Store
              if (typeof updateInstallment2025 === 'function') {
                // إذا كانت دالة المخزن تتطلب الـ index أو الاسم الفريد لعمل التحديث:
                updateInstallment2025(selectedRow.name, finalValues);
              } else {
                // حل بديل فوري ومباشر في حال لم تكن الدالة مهيأة بالكامل بالـ store:
                installments2025[editingIndex] = finalValues;
                useStore.setState({ installments2025: [...installments2025] });
              }
