...
export default function EditModal({
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center sm:p-3" onClick={onClose}>
      <div
      ...
      >
        <div className="bg-gradient-to-l from-primary to-primary/80 text-primary-foreground p-4 rounded-t-2xl sm:rounded-t-xl flex items-center justify-between sticky top-0 z-10">
          ...
          <button onClick={onClose} className="text-3xl leading-none w-10 h-10 flex items-center justify-center -m-2">×</button>
        </div>
        <div className="p-4 sm:p-5 grid grid-cols-1 md:grid-cols-3 gap-3 flex-1">
          {fields.map((f) => (
            <div key={f.key} className={f.colSpan === 3 ? "md:col-span-3" : f.colSpan === 2 ? "md:col-span-2" : ""}>
        ...
        >
          <button onClick={onClose} className="flex-1 sm:flex-none px-4 py-3 border rounded-lg">إلغاء</button>
          <button onClick={() => onSave(data)} className="flex-1 sm:flex-none px-5 py-3 bg-primary text-primary-foreground rounded-lg font-semibold">حفظ</button>
        </div>
      </div>
    </div>
  );
