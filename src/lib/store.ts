...
// دالة تحديث خريطة الإيرادات بناءً على الرموز المدخلة يدوياً
const recalculateRevenueMap = (accounts: Account[]): RevenueMap => {
  const newRevenue: RevenueMap = {};
  accounts.forEach((acc) => {
    // إذا كان السجل يحتوي على رمز إيراد مدخل يدوياً ومبلغ إيرادات أكبر من صفر
...
export const useStore = create<State>()(
  persist(
    (set, get) => ({
      ...
      // عمليات الحسابات الجارية (تتكامل مع الحماية اليدوية لرمز الإيراد والمصروفات)
      addAccount: (a) => {
        const item: Account = {
          ...a,
          id: uid(),
      ...
      updateInstallment: (index, i, year) => set((s) => { const key = year === '2025' ? 'installments2025' : 'installments'; return { [key]: s[key].map((inst, idx) => idx === index ? recalcInstallment({ ...inst, ...i }) : inst) }; }),
      updateInstallmentPayment: (index, month, amount, year) => set((s) => { const key = year === '2025' ? 'installments2025' : 'installments'; return { [key]: s[key].map((inst, idx) => idx === index ? recalcInstallment({ ...inst, payments: { ...inst.payments, [month]: amount } }) : inst) }; }),
      deleteInstallment: (index, year) => set((s) => { const key = year === '2025' ? 'installments2025' : 'installments'; return { [key]: s[key].filter((_, i) => i !== index) }; }),
      ...
      addCustomColumn: (id, col) => set((s) => ({ customTabs: s.customTabs.map((t) => t.id === id && !t.columns.includes(col) ? { ...t, columns: [...t.columns, col] } : t) })),
      removeCustomColumn: (id, col) => set((s) => ({ customTabs: s.customTabs.map((t) => t.id === id ? { ...t, columns: t.columns.filter((c) => c !== col) } : t) })),
      addCustomRow: (id, row) => set((s) => ({ customTabs: s.customTabs.map((t) => t.id === id ? { ...t, rows: [...t.rows, row] } : t) })),
      ...
      getOverdueInstallments: (year) => { const key = year === '2025' ? 'installments2025' : 'installments'; return get()[key].filter((i: Installment) => i.remaining > 0); },
      getInstallmentByIndex: (index, year) => { const key = year === '2025' ? 'installments2025' : 'installments'; return get()[key][index]; },
    }),
    { name: "majlis-yemen-v1", version: 2 }
  )
);

// تصدير الـ Hooks المخصصة لسهولة الاستخدام في المكونات
